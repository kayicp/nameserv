...
dfx deploy accountname_backend --no-wallet --ic --specified-id $NAMER_ID --argument "(
  variant {
    Init = record {
      cmc = \"$CMC_ID\";
      duration = record {
      ...
      service_provider = principal \"$GRANT_PRINCIPAL\";
      name = record {
        duration = record {
          packages = vec {
...
dfx deploy accountname_frontend --no-wallet --ic --specified-id $FRONTEND
